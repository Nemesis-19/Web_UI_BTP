...
class LM(sb.core.Brain):
    ...
    def compute_forward(self, batch, stage):
        ...
        tokens_closed, _ = batch.tokens_closed
        pred = self.hparams.model(tokens_closed)
        return pred
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        """
        batch = batch.to(self.device)
        tokens_closed, tokens_len = batch.tokens_closed
        loss = self.hparams.compute_cost(
            predictions, tokens_closed, length=tokens_len
        )
        return loss
...
def dataio_prepare(hparams):
    ...
    @sb.utils.data_pipeline.provides("text", "tokens_closed")
    def text_pipeline(text):
        """Defines the pipeline that processes the input text."""
        yield text
        tokens_list = tokenizer.encode_as_ids(text)
        tokens_closed = torch.LongTensor([hparams["bos_index"]] + (tokens_list) + [hparams["eos_index"]])
        yield tokens_closed
    ...
    sb.dataio.dataset.set_output_keys(
        datasets, ["id", "text", "tokens_closed"],
    )
